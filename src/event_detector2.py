...
class EventDetector:
    ...
    def detect_fixation(self, times_to_run=3, blink_length=3):
                    ...
                    if len(gaze_buffer_x) > 120:
                        ...
                        if dispersion < dispersion_thresh:
                            ...
                            if fixation_count <= times_to_run - 1:
                                gaze_buffer_x = []
                                gaze_buffer_y = []
                            elif fixation_count >= times_to_run:
                                x_fixation_pos = sum(
                                    gaze_buffer_x) / float(len(gaze_buffer_x))
                                y_fixation_pos = sum(
                                    gaze_buffer_y) / float(len(gaze_buffer_y))
                                return ['fixation', [x_fixation_pos, y_fixation_pos]]
    ...
    def grab_bgr_frame(self):
        ...
        if msg['format'] == 'bgr':
            frame = np.frombuffer(
                raw_recv[2], dtype=np.uint8).reshape(720, 1280, 3)
        else:
